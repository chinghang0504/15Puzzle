...
public class PuzzleSolver3 implements PuzzleSolverResult, Runnable {
    ...
    private class PuzzleState {
        ...
        public void updateManhattanDistance() {
            PuzzlePosition subgoalPuzzlePosition = getGoalPuzzlePosition(subgoalTile);

            if (subgoalPuzzlePosition.ROW < dimension - 2) {
                if (subgoalPuzzlePosition.COL < dimension - 2) {
                    updateManhattanDistanceOneTile();
                } else {
                    updateManhattanDistanceTwoTiles(true);
                }
            } else if (subgoalPuzzlePosition.COL < dimension - 2) {
        ...
        private void updateManhattanDistanceOneTile() {
            PuzzlePosition subgoalPuzzlePosition = getGoalPuzzlePosition(subgoalTile);
            PuzzlePosition currPuzzlePosition = null;
            ...
            for (int i = 0; i < dimension && keepSearching; i++) {
                for (int j = 0; j < dimension; j++) {
                    if (board[i][j] == emptyTile) {
                        ...
                        currPuzzlePosition = new PuzzlePosition(i, j);
                    }
                    
                    if (emptyTilePuzzlePosition != null && currPuzzlePosition != null) {
                        keepSearching = false;
                        break;
                    }
                }
            ...
            if (currPuzzlePosition.equals(subgoalPuzzlePosition)) {
                manhattanDistance = 0;
            } else {
                manhattanDistance += PuzzlePosition.getManhattanDistance(currPuzzlePosition, subgoalPuzzlePosition);
                manhattanDistance += PuzzlePosition.getManhattanDistance(currPuzzlePosition, emptyTilePuzzlePosition);
            }
        }
        ...
        private void updateManhattanDistanceTwoTiles(boolean isRow) {
            ...
            PuzzlePosition subgoalPuzzlePosition1 = getGoalPuzzlePosition(subgoalTile);
            PuzzlePosition currPuzzlePosition1 = null;
            PuzzlePosition subgoalPuzzlePosition2 = getGoalPuzzlePosition(subgoalTile2);
            PuzzlePosition currPuzzlePosition2 = null;
            ...
            for (int i = 0; i < dimension && keepSearching; i++) {
                for (int j = 0; j < dimension; j++) {
                    if (board[i][j] == emptyTile) {
                        ...
                        currPuzzlePosition1 = new PuzzlePosition(i, j);
                    } else if (board[i][j] == subgoalTile2) {
                        currPuzzlePosition2 = new PuzzlePosition(i, j);
                    }

                    if (emptyTilePuzzlePosition != null && currPuzzlePosition1 != null && currPuzzlePosition2 != null) {
                        keepSearching = false;
                        break;
                    }
                }
            ...
            if (currPuzzlePosition1.equals(subgoalPuzzlePosition1) && currPuzzlePosition2.equals(subgoalPuzzlePosition2)) {
                manhattanDistance = 0;
            } else {
                manhattanDistance += PuzzlePosition.getManhattanDistance(currPuzzlePosition1, subgoalPuzzlePosition1);
                manhattanDistance += PuzzlePosition.getManhattanDistance(currPuzzlePosition2, subgoalPuzzlePosition2);
                manhattanDistance += PuzzlePosition.getManhattanDistance(currPuzzlePosition1, emptyTilePuzzlePosition);
                manhattanDistance += PuzzlePosition.getManhattanDistance(currPuzzlePosition2, emptyTilePuzzlePosition);
                manhattanDistance += PuzzlePosition.getManhattanDistance(currPuzzlePosition1, currPuzzlePosition2);
            }
        }
        ...
        private PuzzlePosition getGoalPuzzlePosition(int tile) {
        ...
        private boolean isValidPuzzlePosition(PuzzlePosition puzzlePosition) {
            // Out of boundary
            if (puzzlePosition.ROW < 0 || puzzlePosition.ROW >= dimension || puzzlePosition.COL < 0 || puzzlePosition.COL >= dimension) {
                return false;
            }

            // Subgoal tile boundary
            PuzzlePosition subgoalPuzzlePosition = getGoalPuzzlePosition(subgoalTile);
            if (puzzlePosition.ROW < subgoalPuzzlePosition.ROW) {
                return false;
            }
            if (puzzlePosition.ROW == subgoalPuzzlePosition.ROW && puzzlePosition.COL < subgoalPuzzlePosition.COL) {
                return false;
            }
            if (subgoalPuzzlePosition.ROW >= dimension - 2 && puzzlePosition.COL < subgoalPuzzlePosition.COL) {
                return false;
            }
    ...
    // Constructor
    public PuzzleSolver3(String path) {
